...
public class Variable implements Serializable, BshClassManager.Listener {
    ...
    Variable(String name, Type type, Object value, Modifiers modifiers) throws UtilEvalError {
        this.name = name;
        this.type = type;
        this.rawType = bsh.internals.Types.getRawType(type);
        this.modifiers = modifiers;
        ...
        this.value = type != null && type != Object.class && value != null
                        ? this.value = Types.castObject(value, this.rawType, Types.CAST)
                        : value;
    }
    ...
    public Object setValue(Object value) throws UtilEvalError {
        ...
        if (this.hasModifier("final")) {
            if (this.value != null)
                throw new UtilEvalError("Cannot re-assign final variable "+name+".");
            if (this.value == null) // TODO: pq temos isso?
                return null;
        }

        // TODO: testar com o raw type de generics para os base type da definição do generic
        // TODO: should add isJavaCastable() test for strictJava
        // (as opposed to isJavaAssignable())
        if ( this.type != null && this.type != Object.class && value != null )
            return this.value = Types.castObject(value, this.rawType, Types.ASSIGNMENT);
        else
            return this.value = this.value == null ? Primitive.getDefaultValue(this.rawType) : value;
    }
    ...
    */
    ...
    public void setConstant() {
        if (hasModifier("private") || hasModifier("protected"))
            throw new IllegalArgumentException("Illegal modifier for interface field " + this.name + ". Only public static & final are permitted.");
