...
// import static bsh.This.Keys.BSHCONSTRUCTORS; // TODO: make tests to generated class to try to reply it; 'import static' for things that wasn't declared yet
...
// TODO: see the class and try to maintain the methods!
/**
...
public final class This implements java.io.Serializable, Runnable {
    ...
    //         */
    //         BshLocalMethod equalsMethod = Reflect.getMethod(
    ...
    //         */
    //         BshLocalMethod toStringMethod = Reflect.getMethod(
    ...
    //     BshLocalMethod toString = Reflect.getMethod(namespace, "toString", new Class<?>[0]);
    ...
    //     BshLocalMethod bshMethod = Reflect.getMethod(
    ...
    //         for (BshLocalMethod method : namespace.getMethods()) {
    //             BshLocalMethod cloneM = method.clone();
    ...
    //         BshLocalMethod constructor = instanceNameSpace.getMethod(
    ...
    //////////////////////////////////////////////////////////////
    // Just to maintain the signature
    // TODO: see it better later

    @Deprecated // TODO: do we need it?
    public void run() {
        try {
            Reflect.invokeMethod(this, "run", Reflect.ZERO_ARGS, new CallStack(), false);
        } catch (Throwable e) {
    ...
    // TODO: mudar os acessores para private doq for possível!!!
    protected final NameSpace namespace; // TODO: renomear para 'nameSpace' ??
    protected final boolean isNameSpaceWrapper;
    // protected final boolean strictJava;
    
    // Exclusive fields for a This that is a wrapper of an object!
    protected final Object _this;
    protected final Class<?> thisClass; // Note: it's the class to we target when invoke some method
    // protected boolean initing;
    protected final Set<String> finalFieldsSet = new HashSet<>(); // TODO: dar um .clear() when 'constructing = false'
    protected final CheckList<String> finalFieldsToSet;

    This(final NameSpace nameSpace) {
        this.namespace = nameSpace;
        this.isNameSpaceWrapper = true;
        // this.strictJava = false; // Note: a NameSpace wrapper is a non-strictJava feature!
        this._this = null;
        this.thisClass = null;
        this.finalFieldsToSet = null;
    }

    // TODO: remover esse constructor, tlvz privar ? sla, não deveria ser possível criar instâncias de This tão facilmente!
    public This(final Object _this) {
        // Note: we start 'constructing' as 'true' because wer're building it yet, then we change to 'false'
        this(_this, _this.getClass(), true);
        if (!Reflect.isGeneratedClass(this.thisClass)) throw new IllegalArgumentException("Can't create an This");
    }

    // TODO: ver esse 'initing' ?
    // Note: it must be an internal constructor!
    private This(final Object _this, final Class<?> thisClass, boolean initing) {
        // if (!thisClass.isInstance(_this)) throw new IllegalArgumentException();
        // this.namespace = !strictJava ? new NameSpace("") : null; // Note: we create a NameSpace to store dynamic variables ( when we set fields that don't exists in 'this'! )
        this.namespace = new NameSpace(""); // TODO: ver esse nameSpace ai e a string
        this.isNameSpaceWrapper = false;
        ...
        if (initing) {
            final List<String> _finalFieldsToSet = new ArrayList<>();
            // for (BshField field: BshClass.fromGeneratedClass(thisClass).fields);

            this.finalFieldsToSet = new CheckList<>();
        } else {
            this.finalFieldsToSet = new CheckList<>();
        }
    }

    public static boolean isObjectWrapper(Object obj) {
        return obj instanceof This && !((This) obj).isNameSpaceWrapper;
    }

    protected static boolean isNameSpaceWrapper(Object obj) {
        return obj instanceof This && ((This) obj).isNameSpaceWrapper;
    }

    // public void setIniting(boolean initing) {
    //     if (this.isNameSpaceWrapper) throw new IllegalAccessError("Can't set constructing 'cause it's a NameSpace wrapper!");
    //     if (!this.initing) throw new IllegalAccessError("Can't set constructing 'cause it's already false!");
    //     if (initing) throw new IllegalArgumentException("Can't set constructing to true!");
    //     this.initing = false;
    //     this.finalFieldsSet.clear(); // Note: we don't need anymore know what final fields were already defined
    // }

    // protected boolean canSetFinalField(String fieldName) {
    //     if (this.isNameSpaceWrapper) throw new IllegalAccessError("Can't validate if can set a final field 'cause it's a NameSpace wrapper!");
    //     return this.initing && this.finalFieldsSet.add(fieldName);
    // }

    protected This getSuper() {
        // TODO: verificar a questão de erros quando não for possível resolver o 'super'
        if (this.isNameSpaceWrapper) {
            // Note: 'super' for NameSpace wrapper is basically a wrapper for its parent
            final NameSpace parent = this.namespace.getParent();
            if (parent == null) throw new IllegalAccessError("Can't resolve resolve the 'super' of This reference!");
            return new This(parent);
        } else {
            final Class<?> superClass = this.thisClass.getSuperclass();
            if (superClass == null) throw new IllegalAccessError("Can't resolve resolve the 'super' of This reference!");
            return new This(this._this, superClass, false);
        }
    }

    // protected abstract <T> T getField(String name) throws UtilEvalError;
    // protected abstract <T> void setField(String name, T value) throws UtilEvalError;
    // protected abstract <T> T invokeMethod(String method, Object ...args) throws UtilEvalError, TargetError;

    // protected abstract Object unwrap();

    // // TODO: tirar esse ThisFromNameSpace
    // protected static class ThisFromNameSpace extends This {
    //     private final NameSpace nameSpace = null;
    //     // private final boolean strictJava = false; // Note: 'strictJava' is always false here because there is no 'ThisFromNameSpace' instance in strict Java mode!
    //     protected NameSpace unwrap() { return this.nameSpace; }
    // }

    // TODO: fazer validações para o field não ser static
    // TODO: como fica os 'this' fora de classes ?
    // TODO: como fica o 'this' para métodos static ?
    // TODO: como saber qual o tipo do dynamicField ?
    // TODO: testes para manter a tipagem primitiva e n primitiva!
    // TODO: fazer teste se a ordem de campos está correta!
    // TODO: precisa de um .setAccessible() ???

    // // TODO: melhorar essa classe!
    // protected static class ThisFromObject extends This {
    //     private final Object thisObj; // TODO: The instance being wrapped
    ...
    //     protected ThisFromObject(Object thisObj) { this.thisObj = thisObj; }
