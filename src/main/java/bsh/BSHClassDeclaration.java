...
class BSHClassDeclaration extends SimpleNode
{
    ...
    private Class<?> getSuperClass(CallStack callStack, Interpreter interpreter) throws EvalError {
        ...
        final BSHName superNode = this.jjtGetChild(0);
        final Class<?> superClass = superNode.toClass(callStack, interpreter);
    ...
    private Class<?> generateClass(final CallStack callStack, final Interpreter interpreter) throws EvalError {
        ...
        String name = this.getName(callStack.top());
        Class<?> superClass = this.getSuperClass(callStack, interpreter);
        Class<?>[] interfaces = this.getInterfaces(callStack, interpreter);
        ...
        final List<BshField> fields = new ArrayList<>();
        final List<BshMethod> methods = new ArrayList<>();
        final List<BshConstructor> constructors = new ArrayList<>();

        // BSHBlock block = (BSHBlock) jjtGetChild(child);
        int offset = this.extend ? 1+this.numInterfaces : this.numInterfaces;
        ...
        for (int i = offset; i < children.length; i++) {
            ...
            if (n instanceof BSHTypedVariableDeclaration) { // TODO: make test for 'loose typed' fields!
                BSHTypedVariableDeclaration node = (BSHTypedVariableDeclaration) n;
                ...
                BSHMethodDeclaration node = (BSHMethodDeclaration) n;
        ...
        NameSpace nameSpace = new NameSpace(callStack.top(), "");
