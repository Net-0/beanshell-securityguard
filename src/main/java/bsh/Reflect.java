...
public final class Reflect {
    ...
    protected static Object getField(Object thisArg, String fieldName, CallStack callStack) throws ReflectError, SecurityError, NoSuchFieldException {
        ...
        if (This.isNameSpaceWrapper(thisArg)) {
            NameSpace ns = ((This) thisArg).namespace;
            try {
                Object value = ns.getVariable(fieldName);
                if (value == Primitive.VOID) throw new NoSuchFieldException();
                return value;// TODO: oq fazer caso retorne Primitive.VOID ??
            } catch (UtilEvalError t) { // TODO: não deveria ter de fazer esse catch para UtilEvalError!!!!!
    ...
    protected static void setField(Object thisArg, String fieldName, Object value, CallStack callStack) throws ReflectError, UtilEvalError {
        if (thisArg == Primitive.NULL) // TODO: validar thisArg == null tb ?
            throw new ReflectError("Attempt to set field '" +fieldName+"' on null value"); // TODO: verificar a mensagem!

        if (thisArg instanceof This) {
            NameSpace ns = ((This) thisArg).namespace;
        ...
        try {
            ...
            if (Reflect.isGeneratedClass(_class) && Reflect.isFinal(field) && thisArg instanceof This)
                setAccessible = field.getDeclaringClass() == _class && ((This) thisArg).canSetFinalField(fieldName);

            if (setAccessible) field.setAccessible(true);
            field.set(Primitive.unwrap(thisArg), value);
    ...
    protected static Object getStaticField(Class<?> _class, String fieldName, CallStack callStack) throws ReflectError, SecurityError, NoSuchFieldException {
    ...
    protected static <T> T construct(Class<T> _class, Object[] args, CallStack callStack) throws ReflectError, SecurityError, TargetError, NoSuchMethodException {
        ...
        if (constructor == null) {
            final String msg = String.format("There is no such constructor: %s(%s)", _class.getName(), String.join(", ", Types.prettyNames(argsTypes)));
            throw new NoSuchMethodException(msg);
        }

        final Object[] _args = collectArguments(args, argsTypes, constructor.isVarArgs());
        ...
        try {
            ...
            T result = (T) constructor.newInstance(_args); // TODO: testar com var-args!
        ...
        try {
            ...
            if (!isVarArgs) {
                for (int i = 0; i < paramTypes.length; i ++)
                    collected[i] = Primitive.unwrap(Types.castObject(args[i], paramTypes[i], Types.BSH_ASSIGNABLE));
                return collected;
            }
            ...
            for (int i = 0; i < varArgs.length; i++)
                varArgs[i] = (V) Primitive.unwrap(Types.castObject(args[paramTypes.length-1 + i], varArgsType, Types.BSH_ASSIGNABLE));
    ...
    protected static Object invokeLocalMethod(NameSpace nameSpace, String methodName, Object[] args, CallStack callStack) throws ReflectError, SecurityError, TargetError, NoSuchMethodException {
        Class<?>[] argsTypes = Types.getTypes(args);
        BshLocalMethod method = nameSpace.getMethod(methodName, argsTypes);

        if (method == null) {
            String msg = String.format("There is no such local method: %s(%s)", methodName, String.join(", ", Types.prettyNames(argsTypes)));
            throw new NoSuchMethodException(msg);
        }
        ...
        return method.invoke(callStack, _args);
    ...
    // TODO: trocar o callingNS por CallStack, e pegar o callStack.top()
    // TODO: esse método retorna Primitive
    // TODO: invocar metodo de nameSpace caso o thisArg seja This.isFromNameSpace == true ??
    public static Object invokeMethod(Object thisArg, String methodName, Object[] args, CallStack callStack) throws ReflectError, SecurityError, TargetError, NoSuchMethodException {
        ...
        if (This.isNameSpaceWrapper(thisArg)) {
            ...
            return Reflect.invokeLocalMethod(ns, methodName, args, callStack);
        }
        ...
        if (method == null) {
            final String msg = String.format("There is no such method: %s.%s(%s)", _class.getName(), methodName, String.join(", ", Types.prettyNames(argsTypes)));
            throw new NoSuchMethodException(msg);
        }
    ...
    protected static Object invokeStaticMethod(Class<?> _class, String methodName, Object[] args, CallStack callStack) throws ReflectError, SecurityError, TargetError, NoSuchMethodException {
        ...
        if (method == null) {
            final String msg = String.format("There is no such static method: %s.%s(%s)", _class.getName(), methodName, String.join(", ", Types.prettyNames(argsTypes)));
            throw new ReflectError(msg);
        }
    ...
    */
    ...
    public static Object getStaticBeanProperty(Class<?> _class, String propName, CallStack callStack) throws ReflectError, SecurityError, TargetError {

        try { // e.g., MyClass.getInfo()
            final String getterName = "get" + propName.substring(0, 1).toUpperCase() + propName.substring(1);
            return Reflect.invokeStaticMethod(_class, getterName, Reflect.ZERO_ARGS, callStack);
        } catch (NoSuchMethodException e) {}

        try { // e.g., MyClass.isActive()
            final String booleanGetterName = "is" + propName.substring(0, 1).toUpperCase() + propName.substring(1);
            return Reflect.invokeStaticMethod(_class, booleanGetterName, Reflect.ZERO_ARGS, callStack);
        } catch (NoSuchMethodException e) {}
    ...
    public static Object getObjectProperty(Object obj, Object key, CallStack callStack) throws ReflectError, SecurityError, TargetError {
        ...
        if (!(key instanceof String)) return Primitive.VOID;
        ...
        try {
            return Reflect.invokeMethod(obj, getterName, Reflect.ZERO_ARGS, callStack);
        } catch (NoSuchMethodException e) {}

        try {
            return Reflect.invokeMethod(obj, booleanGetterName, Reflect.ZERO_ARGS, callStack);
        } catch (NoSuchMethodException e) {}

        return Primitive.VOID;
    }

    // TODO: ver para remover isso!
    public static final <K, V> Entry<K, V> getEntryForKey(K key, Entry<K, V>[] entries) {
    ...
    public static Object setObjectProperty(
        ...
        if (obj instanceof Entry) switch(propName) {
            case "val": case "value":
    ...
    public static Object setObjectProperty(
        ...
        if (Entry[].class.isAssignableFrom(cls)) {
            throw new RuntimeException("Not implemented yet!");
            // return getEntryForKey(propName, (Entry[]) obj).setValue(Primitive.unwrap(value));
        }
