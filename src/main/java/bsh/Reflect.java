...
public final class Reflect {
    ...
    */
    ...
    protected static Object getNameSpaceVariable(NameSpace nameSpace, String variableName, CallStack callStack, boolean strictJava) throws ReflectError, EvalError, UtilEvalError, NoSuchElementException {
        ...
        for (NameSpace ns = nameSpace; ns != null; ns = ns.getParent()) {
            ...
            if (ns.declaringClass instanceof Class<?>)
                try {
                    return Reflect.getStaticField((Class<?>) ns.declaringClass, variableName, callStack, strictJava);
                } catch (NoSuchFieldException e) {}

            // TODO: precisamos disso ? Isso dá problemas no bsh.EnumTest#enum_with_static_method
            // if (!strictJava) {
    ...
    protected static Object setNameSpaceVariable(NameSpace nameSpace, String variableName, Object value, CallStack callStack, boolean strictJava) throws ReflectError, EvalError, UtilEvalError, NoSuchElementException {
        ...
        for (NameSpace ns = nameSpace; ns != null; ns = ns.getParent()) {
            ...
            //     return variable;
            if (ns.hasLocalVariable(variableName))
                return ns.setLocalVariable(setterName, null, value, null);

            // Try to set a instance field
            if (ns._this != null)
                try {
                    // return Reflect.getField(ns._this, variableName, callStack, strictJava);
                    return Reflect.setField(ns._this, variableName, value, callStack, strictJava);
                } catch (NoSuchFieldException e) {}

            // Try to set a static field
            if (ns.declaringClass instanceof Class<?>)
                try {
                    // return Reflect.getStaticField(ns.declaringClass, variableName, callStack, strictJava);
                    return Reflect.setStaticField((Class<?>) ns.declaringClass, variableName, value, callStack);
                } catch (NoSuchFieldException e) {}

            // TODO: testar a ordem de execução do invokeNameSpaceMethod, se é necessário, etc...
            // Try to set a property
            if (!strictJava) {
                try { // e.g., myObj.setId( 123 )
                    Reflect.invokeNameSpaceMethod(ns, setterName, new Object[] { value }, callStack, strictJava);
                    return value;
                } catch (NoSuchMethodException e) {}
            }
        }
        ...
        // throw new NoSuchElementException(); // TODO: see it!
        return nameSpace.setLocalVariable(variableName, null, value, null);
    }
    ...
    protected static Object getField(Object thisArg, String fieldName, CallStack callStack, boolean strictJava) throws ReflectError, EvalError, UtilEvalError, NoSuchFieldException {
        ...
        System.out.printf("Reflect.getField(%s, %s)\n", thisArg.getClass(), fieldName);
        ...
        // TODO: ver para usar isso ?
        // throw new ReflectError.NoSuchFieldError(_class, fieldName);
        if (field == null) {
            final String msg = String.format("No such field: %s.%s", _class.getName(), fieldName);
    ...
    protected static Object setField(Object thisArg, String fieldName, Object value, CallStack callStack, boolean strictJava) throws ReflectError, EvalError, UtilEvalError, NoSuchFieldException {
        ...
        System.out.println("---------------------------------------------");
        System.out.println("Reflect.setField() -> thisArg: " + thisArg);
        System.out.println("Reflect.setField() -> fieldName: " + fieldName);
        System.out.println("---------------------------------------------");
        ...
        if (Reflect.isFinal(field) && This.isObjectWrapper(thisArg) && ((This) thisArg).finalFieldsSet.add(fieldName))
    ...
    protected static Object getStaticField(Class<?> _class, String fieldName, CallStack callStack, boolean strictJava) throws ReflectError, UtilEvalError, NoSuchFieldException {
        // TODO: validar se _class == null ?
        final NameSpace callingNS = callStack.top();
        final Field field = BshClassManager.memberCache.get(_class).findStaticField(fieldName, callingNS);

        System.out.printf("Reflect.getStaticField(%s, %s)\n", _class, fieldName);
        ...
        try {
            ...
            // TODO: adicionar esse catch em todos os métodos, trocando na assinatura o 'SecurityEvalError' por 'UtilEvalError'
            throw new UtilTargetError(t);
        }
    }

    protected static Object setStaticField(Class<?> _class, String fieldName, Object value, CallStack callStack) throws ReflectError, SecurityError, NoSuchFieldException, UtilEvalError {
        final NameSpace callingNS = callStack.top();
        final Field field = BshClassManager.memberCache.get(_class).findStaticField(fieldName, callingNS);
    ...
    protected static <T> T construct(Class<T> _class, Object[] args, CallStack callStack) throws ReflectError, EvalError, UtilEvalError, NoSuchMethodException {
        ...
        try {
            ...
            constructor.setAccessible(true); // TODO: isso n pode dar problemas ? tipo, tentar forçar um constructor de uma classe do Java à ser acessível
            // TODO: precisa dar Primitve.unwrap() nos args ???
            final T instance = (T) constructor.newInstance(_args); // TODO: testar com var-args!
    ...
    private static <V> Object[] collectArguments(Object[] args, Class<?>[] paramTypes, boolean isVarArgs) {
        try {
            ...
            // Cast the first arguments
            for (int i = 0; i < paramTypes.length - 1; i ++)
                collected[i] = Primitive.unwrap(Types.castObject(args[i], paramTypes[i], Types.CAST));

            final Class<V[]> varArgsArrayType = (Class<V[]>) paramTypes[paramTypes.length-1];
            final Class<V> varArgsType = (Class<V>) varArgsArrayType.getComponentType();
            final Class<?> lastArgType = Types.getType(args[args.length-1]);

            // TODO: fazer o teste: String.format("teste: %s", new String[] { "abc", "def" }) -> "teste: abc"
            // TODO: fazer o teste: String.format("teste: %s", new Object[] { "abc", "def" }) -> "teste: abc"
            // TODO: teste com loose-type array ??
            // Is invoking a var-args method with already grouped var-args ?
            // TODO: rever os round == Types.BSH_ASSIGNABLE
            // TODO: rever os Types.CAST!
            if (paramTypes.length == args.length && lastArgType != null && lastArgType.isArray() && Types.isAssignable(lastArgType.getComponentType(), varArgsType, Types.BSH_ASSIGNABLE)) {
                collected[paramTypes.length-1] = Types.castObject(args[args.length-1], varArgsArrayType, Types.CAST);
            } else {
                // Cast and group the last arguments
    ...
    protected static Object invokeNameSpaceMethod(NameSpace nameSpace, String methodName, Object[] args, CallStack callStack, boolean strictJava) throws ReflectError, EvalError, UtilEvalError, NoSuchMethodException {
        ...
        for (NameSpace ns = nameSpace; ns != null; ns = ns.getParent()) {
            ...
            if (localMethod != null) {
                ...
                // TODO: ver como ficam os erros dps dess mexidas no local-methods e class-methods
                // return localMethod.invoke(callStack, _args);
                return localMethod.body.apply(callStack, _args);
            }

            // Try class method ( static and non-static )
            if (ns.declaringClass instanceof Class<?>)
                try {
                    return Reflect.invokeStaticMethod((Class<?>) ns.declaringClass, methodName, args, callStack, strictJava);
                } catch (NoSuchMethodException e) {}
            if (ns._this != null)
                try {
                    return Reflect.invokeMethod(ns._this, methodName, args, callStack, strictJava);
                } catch (NoSuchMethodException e) {}
            ...
            for (final Name className: classNames)
                try {
                    return Reflect.invokeStaticMethod(className._toClass(), methodName, args, callStack, strictJava);
                } catch (NoSuchMethodException e) {
        ...
        // if (localMethod == null) {
            String msg = String.format("Command not found: %s(%s)", methodName, String.join(", ", Reflect.getTypesNames(argsTypes)));
            throw new NoSuchMethodException(msg);
        // }
    ...
    public static Object invokeMethod(Object thisArg, String methodName, Object[] args, CallStack callStack, boolean strictJava) throws ReflectError, EvalError, UtilEvalError, NoSuchMethodException {
        ...
        if (method == null) {
            final String msg = String.format("No such method: %s.%s(%s)", _class.getName(), methodName, String.join(", ", Reflect.getTypesNames(argsTypes)));
            throw new NoSuchMethodException(msg);
        }
        ...
        try {
            ...
            final Object result = method.invoke(_this, _args); // TODO: testar com var-args!
    ...
    public static Object invokeStaticMethod(Class<?> _class, String methodName, Object[] args, CallStack callStack, boolean strictJava) throws ReflectError, EvalError, UtilEvalError, NoSuchMethodException {
        ...
        System.out.println("------------------------------------------------------------------------");
        System.out.printf("Reflect.invokeStaticMethod(%s, %s, %s)\n", _class, methodName, args.length);
        Thread.dumpStack();
        System.out.println("------------------------------------------------------------------------");

        if (method == null) {
            final String msg = String.format("No such static method: %s.%s(%s)", _class.getName(), methodName, String.join(", ", Reflect.getTypesNames(argsTypes)));
            throw new NoSuchMethodException(msg);
        }
        ...
        try {
            ...
            method.setAccessible(true);
            // if (!Reflect.isPublic(method)) method.setAccessible(true);
            final Object result = method.invoke(null, _args); // TODO: testar com var-args!
            method.setAccessible(wasAccessible);
            // if (!Reflect.isPublic(method)) method.setAccessible(false);
            // method.setAccessible(false);
            return Primitive.wrap(result, method.getReturnType());
    ...
    // TODO: ver melhor os métodos de findMostSpecific e suas assinaturas
    public static <T> T findMostSpecificInvocable(Class<?>[] idealMatch, T[] invocables, Function<T, Class<?>[]> paramTypesGetter, Predicate<T> isVarArgs) {
        final int match = findMostSpecificInvocableIndex(idealMatch, invocables, paramTypesGetter, isVarArgs);
        return match == -1 ? null : invocables[match];
    }
    ...
    // public static final int findMostSpecificInvocableIndex(Class<?>[] idealMatch, BshMethod[] invocables) {

    //     Reflect.findMostSpecificInvocableIndex(idealMatch, invocables, (BshMethod m) -> {
    //         // m
    //         m.getParameterTypes();
    //         return null;
    //     }, null);
    //     return 0;
    //     // return Reflect.findMostSpecificInvocableIndex(idealMatch, invocables, BshMethod::getParameterTypes, BshMethod::isVarArgs);
    // }

    // public static final int findMostSpecificInvocableIndex(Class<?>[] idealMatch, Iterable<BshConstructor> invocables) {
    //     return Reflect.findMostSpecificInvocableIndex(idealMatch, invocables, BshConstructor::getParameterTypes, BshConstructor::isVarArgs);
    // }

    // TODO: tlvz adicionar no Primitive de NULL o type original esperado para aquele dado ?
    // TODO: adicionar uma validação para poder achar somente um match ? Não é perigoso poder chamar outro método ou constructor devido à ambiguidade ? e.g.: ambiguidade devido um valor ser null
    public static final int findMostSpecificInvocableIndex(Class<?>[] idealMatch, BshConstructor[] invocables) {
        return Reflect.findMostSpecificInvocableIndex(idealMatch, invocables, BshConstructor::getParameterTypes, BshConstructor::isVarArgs);
        // int match = Reflect.findMostSpecificInvocableIndex(idealMatch, invocables, BshConstructor::getParameterTypes, BshConstructor::isVarArgs);
        // System.out.println("------------------------------------------------------------------------------------------");
        // System.out.println("Reflect.findMostSpecificInvocableIndex(Class<?>[] idealMatch, BshConstructor[] invocables)");
        // System.out.println("  -> idealMatch: " + Arrays.asList(idealMatch));
        // System.out.println("  -> invocables: " + Arrays.asList(invocables));
        // System.out.println("  -> match: " + match);
        // System.out.println("------------------------------------------------------------------------------------------");
        // return match;
    }

    public static final int findMostSpecificInvocableIndex(Class<?>[] idealMatch, Constructor[] invocables) {
        return Reflect.findMostSpecificInvocableIndex(idealMatch, invocables, Constructor::getParameterTypes, Constructor::isVarArgs);
    }

    // TODO: re-implementar esse método para ele ser menor ?
    // public static final <T> int findMostSpecificInvocableIndex(Class<?>[] idealMatch, Iterable<T> invocables, Function<T, Class<?>[]> paramTypesGetter, Predicate<T> isVarArgs) {
    public static final <T> int findMostSpecificInvocableIndex(Class<?>[] idealMatch, T[] invocables, Function<T, Class<?>[]> paramTypesGetter, Predicate<T> isVarArgs) {
        // for (int i=0; i<executables.size(); i++)
        //     Interpreter.debug("  "+i+":"+executables.get(i).toString()+" "+executables.get(i).getClass().getName());
        ...
        i=0;
        for(T invocable : invocables) {
            final Class<?>[] parameterTypes = paramTypesGetter.apply(invocable);
    ...
    */
    // TODO: rever esse método, como fica dois métodos semelhantes ? e.g.: func(int a) e func(Integer a)
	...
	public static Object setObjectProperty(Object obj, Object key, Object value, CallStack callStack, boolean strictJava) throws ReflectError, EvalError, UtilEvalError, NoSuchElementException {
        ...
        final String msg = String.format("No such property in %s for %s", cls.getName(), key);
    ...
    public static final Class<?> getType(Object arg) {
        return Types.getType(arg);
    }

    /** Get the Java types of the arguments. */
    public static final Class<?>[] getTypes(Object[] args) {
        return Types.getTypes(args);
    }

    // TODO: utilizar Class#getCanonicalName() para ter nomes mais bonitos para os usuários ? ou melhor o nome normal assim como o java já faz ( eu acho ) ?
    // TODO: mover esse método ?
    ...
    // TODO: utilizar Class#getCanonicalName() para ter nomes mais bonitos para os usuários ? ou melhor o nome normal assim como o java já faz ( eu acho ) ?
    // TODO: mover esse método ?
    /** Return the names of types */
    public static final String[] getTypesNames(Type[] types) {
        final String[] names = new String[types.length];
        for (int i = 0; i < types.length; i++)
            names[i] = Reflect.getTypeName(types[i]);
        return names;
    }

    // public static final <T> T cast(Object obj, Class<T> type) throws UtilEvalError {
    //     return (T) Types.castObject(obj, type, Types.CAST);
    // }

    public static final <T> T assign(Object obj, Class<T> type) throws UtilEvalError {
        return (T) Types.castObject(obj, type, Types.ASSIGNMENT);
    }
}
