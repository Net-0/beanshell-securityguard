    ...
    public LHS toLHS(CallStack callStack, Interpreter interpreter) throws EvalError {
        ...
        try {
            ...
            if (this.jjtGetNumChildren() == 1) {
                ...
                if (lastDotIndex > 0) {
                    final String baseName = name.substring(0, lastDotIndex);
                    final String memberName = name.substring(lastDotIndex + 1);
                    final Object baseObj = nameSpace.getNameResolver(baseName).toObject(callStack, interpreter);
                    return new LHS(baseObj, memberName);
                }

                // e.g. abc = 10
                return new LHS(nameSpace, name);
            }
