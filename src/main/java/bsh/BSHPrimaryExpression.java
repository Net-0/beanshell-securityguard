    ...
    public LHS toLHS(CallStack callStack, Interpreter interpreter) throws EvalError {
        ...
        try {
            ...
            if (this.jjtGetNumChildren() == 1) {
                ...
                final BSHAmbiguousName ambiguousName = this.jjtGetChild(0);
    
                if (Name.isCompound(ambiguousName.name)) {
                    final String prefix = Name.prefix(ambiguousName.name);
                    final String memberName = Name.suffix(ambiguousName.name, 1);
    
                    final Object baseObj = nameSpace.getNameResolver(prefix).toObject(callStack, interpreter);
                    return new LHS(baseObj, memberName);
                }

                return new LHS(nameSpace, ambiguousName.name);
            }
