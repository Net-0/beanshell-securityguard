...
public abstract class BshLambda {
    ...
    private static final WeakHashMap<Class<?>, BshLambda> dummyTypesInstances = new WeakHashMap<>();

    final Node expressionNode;
    final Class<?> dummyType;

    private BshLambda(Node expressionNode) {
        this.expressionNode = expressionNode;
        this.dummyType = BshLambda.generateDummyType();

        BshLambda.dummyTypesInstances.put(this.dummyType, this);
    }
    ...
    static BshLambda fromLambdaExpression(Node expressionNode, NameSpace declaringNameSpace, Modifiers[] paramsModifiers, Class<?>[] paramsTypes, String[] paramsNames, SimpleNode bodyNode) {
    ...
    static BshLambda fromMethodReference(Node expressionNode, Object thisArg, String methodName) {
        return new BshLambdaFromMethodReference(expressionNode, thisArg, methodName);
    }

    // Generate a dummy type
    private static volatile int typeCount = 1;
    private static Class<?> generateDummyType() {
        final String interfaceName = BshLambda.class.getName() + "Type" + (BshLambda.typeCount++);
    ...
    protected abstract boolean isAssignableImpl(Method to, int round) throws EvalError, UtilEvalError;

    static boolean isDummyType(Class<?> type) {
        return BshLambda.dummyTypesInstances.containsKey(type);
    }
    ...
    static boolean isAssignable(Class<?> from, Class<?> to, int round) {
        BshLambda lambda = BshLambda.dummyTypesInstances.get(from);
        if (lambda == null) return false;
        Method method = BshLambda.methodFromFI(to);

        try {
            return lambda.isAssignableImpl(method, round);
        } catch (UtilEvalError e) {
            throw new RuntimeEvalError(e.toEvalError(lambda.expressionNode, null));
        } catch (EvalError e) {
            throw new RuntimeEvalError(e);
        }
    }
    ...
    static Method methodFromFI(Class<?> functionalInterface) {
    ...
    static <T> Class<T> getClassForFI(Class<T> fi) {
    ...
    private static class BshLambdaFromLambdaExpression extends BshLambda {
        ...
        public BshLambdaFromLambdaExpression(Node expressionNode, NameSpace declaringNameSpace, Modifiers[] paramsModifiers, Class<?>[] paramsTypes, String[] paramsNames, SimpleNode bodyNode) {
        ...
        private NameSpace initNameSpace(Object[] args) throws UtilEvalError {
            ...
            for (int i = 0; i < paramsNames.length; i++) {
                ...
                Variable param = new Variable(this.paramsNames[i], this.paramsTypes[i], args[i], this.paramsModifiers[i]);
                nameSpace.setVariableImpl(param);
            }
            return nameSpace;
        }

        // protected boolean isAssignable(Class<?> fi, int round) {
        //     Method fiMethod = BshLambda.methodFromFI(fi);
        //     Class<?>[] fiParamsTypes = fiMethod.getParameterTypes();
        //     if (fiParamsTypes.length != this.paramsTypes.length) return false;

        //     // TODO: validate the return type of 'this.bodyNode' ???
        //     return Types.isSignatureAssignable(this.paramsTypes, fiParamsTypes, round);
        // }

        protected boolean isAssignableImpl(Method to, int round) throws EvalError, UtilEvalError {
            Class<?>[] toParamsTypes = to.getParameterTypes();
            Class<?> toReturnType = to.getReturnType();

            if (this.paramsTypes.length != toParamsTypes.length || !Types.isSignatureAssignable(this.paramsTypes, toParamsTypes, round))
                return false;

            NameSpace bodyNameSpace = this.initTypedNameSpace(toParamsTypes);
            Class<?> fromReturnType = this.bodyNode.getEvalReturnType(bodyNameSpace);
            System.out.println("BshLambda -> fromReturnType: " + fromReturnType);
            System.out.println("BshLambda -> toReturnType: " + toReturnType);

            return Types.isAssignable(fromReturnType, toReturnType, round);
        }

        /** Initialize a name space where all parameters are defined but with standard value */
        private NameSpace initTypedNameSpace(Class<?>[] toParamTypes) throws UtilEvalError {
            NameSpace nameSpace = new NameSpace(this.declaringNameSpace, "LambdaExpression");
            for (int i = 0; i < this.paramsTypes.length; i++) {
                Class<?> paramType = this.paramsTypes[i] == null ? toParamTypes[i] : this.paramsTypes[i];
                Object value = paramType.isPrimitive() ? Primitive.getDefaultValue(this.paramsTypes[i]) : null;
                Variable param = new Variable(this.paramsNames[i], this.paramsTypes[i], value, this.paramsModifiers[i]);
                nameSpace.setVariableImpl(param);
            }
            return nameSpace;
        }
    ...
    private static class BshLambdaFromMethodReference extends BshLambda {
        ...
        protected boolean isAssignableImpl(Method to, int round) {
            boolean staticMethod = thisArg instanceof ClassIdentifier;
            Class<?> _class = thisArg instanceof ClassIdentifier ? ((ClassIdentifier) thisArg).clas : thisArg.getClass();

            if (thisArg instanceof ClassIdentifier && methodName.equals("new")) {
    ...
    private static class WrapperGenerator {
         ...
         *
         * public class MyBooleanSupplier implements Callable {
         *  private BshLambda bshLamba;
         *
         ...
         *
         * public class MyBooleanSupplier implements Runnable {
         *  private BshLambda bshLamba;
         *
         *  public void run() {
