...
public abstract class BshLambda {
    ...
    protected abstract boolean isAssignable(Class<?> functionalInterface, int round);
    ...
    protected <T> T convertTo(Class<T> functionalInterface) throws UtilEvalError {
        if (!this.isAssignable(functionalInterface, Types.BSH_ASSIGNABLE))
    ...
    protected static <T> Class<T> getClassForFI(Class<T> fi) {
        ...
        Class<T> _class = LambdaClassGenerator.generateClass(fi);
    ...
    private static class BshLambdaFromLambdaExpression extends BshLambda {
        ...
        private NameSpace initNameSpace(Object[] args) throws UtilEvalError {
            NameSpace nameSpace = new NameSpace(this.declaringNameSpace, "<BshLambda>");
    ...
    private static class BshLambdaFromMethodReference extends BshLambda {
        ...
        protected final Object invokeImpl(Object[] args) throws UtilEvalError, EvalError, TargetError {
            try {
                NameSpace nameSpace = new NameSpace("<BshLambda>");
                CallStack callStack = new CallStack(nameSpace);
                Interpreter interpreter = new Interpreter(nameSpace);
        ...
        protected boolean isAssignable(Class<?> fi, int round) {
            ...
            if (thisArg instanceof ClassIdentifier && methodName.equals("new")) {
                for (Constructor<?> constructor: _class.getConstructors()) {
                    if (!Types.isSignatureAssignable(constructor.getParameterTypes(), fiMethod.getParameterTypes(), round)) continue;
                    if (!Types.isAssignable(_class, fiMethod.getReturnType(), round)) continue;
                    return true;
                }
                return false;
            }
            ...
            for (Method method: _class.getMethods()) {
                ...
                if (!Types.isSignatureAssignable(method.getParameterTypes(), fiMethod.getParameterTypes(), round)) continue;
                if (!Types.isAssignable(method.getReturnType(), fiMethod.getReturnType(), round)) continue;
                return true;
            }
            return false;
        }
     ...
     */
    private static class LambdaClassGenerator {
        ...
        protected static <T> Class<T> generateClass(Class<T> functionalInterface) {
            final String packageName = BshLambda.class.getPackage().getName().replace(".", "/");
            final String encodedFIName = Base64.getEncoder().encodeToString(functionalInterface.getName().getBytes());
            final String internalClassName = String.format("%s/BshLambdaGenerated%s", packageName, encodedFIName);

            byte[] bytes = generateClassBytes(internalClassName, functionalInterface);
            final String className = internalClassName.replace('/', '.');
            return (Class<T>) byteClassLoader.classFromBytes(className, bytes);
        }
        ...
        private static byte[] generateClassBytes(String className, Class<?> functionalInterface) {
            ...
            cw.visit(Opcodes.V11, Opcodes.ACC_PUBLIC, className, null, superPrimitiveName, interfacesPrimitiveNames);
            ...
            writeConstructor(cw, className);

            Method method = methodFromFI(functionalInterface);
            writeMethod(cw, className, method);
