...
class Name implements java.io.Serializable {
    ...
    */
    ...
    synchronized public Object toObject(CallStack callStack, Interpreter interpreter, boolean autoAllocateThis) throws EvalError, UtilEvalError {
        ...
        Class<?> _class = nameSpace._getClassForName(this.name);
        ...
        */
        ...
        if (baseObj == null) {
            if (this.nameParts[0].equals("this")) {
                ...
                try {
                    // System.out.println("-------------------------------------------------------------------------------------");
                    // System.out.println("Name.toObject() -> this.nameParts[0]: " + this.nameParts[0]);
                    // System.out.println("Name.toObject() -> nameSpace.getAllNames(): " + Arrays.asList(nameSpace.getAllNames()));
                    // System.out.println("Name.toObject() -> nameSpace.getName(): " + nameSpace.getName());
                    // System.out.println("-------------------------------------------------------------------------------------");
                    ...
                    for (int j = 0; j < this.nameParts.length && __class == null; j++) {
                        ...
                        __class = nameSpace._getClassForName(className);
        ...
        if (baseObj == null) {
            ...
            // } else if (this.nameParts.length == 1) {
            //     // TODO: fazer mais testes disso
            //     // TODO: fazer um teste de switch-case com o valor sendo null e tentando acessar uma constante de Enum
        ...
        for (int i = startIndex; i < this.nameParts.length; i++) {
            ...
            if (baseObj instanceof ClassIdentifier) { // Resolve class member
                ...
                Class<?> innerClass = nameSpace._getClassForName(innerClassName);
    ...
    // // TODO: converter para toType ? sla, ver essa parada ai
    // synchronized public Class<?> toClass() throws ClassNotFoundException {
    //     // TODO: It shouldn't be here!
    //     if (this.name.equals("var")) return null;

    //     // /* Try straightforward type name first. It can be a full class name, a local class name or even a generic type */
    //     // Class<?> clas = namespace.getClass( evalName );
    //     Class<?> _class = this.nameSpace.getClass(this.name);
    //     if (_class != null) return _class;
    ...
    // TODO: ver uma implementação próprio para Class<?>, pois existem trechos q precisam ser Class e não Type
    synchronized public Class<?> _toClass() throws ClassNotFoundException {
        // // TODO: It shouldn't be here!
        // if (this.name.equals("var")) return null;
        ...
        // throw new ClassNotFoundException(this.name);
        return (Class<?>) this.toType();
    }

    synchronized public Type toType() throws ClassNotFoundException {
        // TODO: It shouldn't be here!
        if (this.name.equals("var")) return null;

        // TODO: resolver o Type do nameSpace
        final Type type = this.nameSpace.getTypeForName(this.name);
        if (type != null) return type;

        // Note: this will try to solve the name as an inner class name
        // e.g., "com.my.package.a.b.c" -> "com.my.package.a.b$c", "com.my.package.a$b$c", "com.my.package$a$b$c", .....
        for (int i = this.nameParts.length-1; i > 0; i--) {
            final String[] baseClassName = Arrays.copyOfRange(this.nameParts, 0, i);
            final String[] innerClassName = Arrays.copyOfRange(this.nameParts, i, this.nameParts.length);
            final String className = String.join(".", baseClassName) + "$" + String.join("$", innerClassName);
            final Type _type = this.nameSpace.getTypeForName(className);
    ...
    synchronized public LHS toLHS(CallStack callStack, Interpreter interpreter) throws EvalError, UtilEvalError {
        ...
        // e.g. abc = 10
        return new LHS(this.nameSpace, this.name, callStack, strictJava);
    }
