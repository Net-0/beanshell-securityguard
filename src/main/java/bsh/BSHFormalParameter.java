...
class BSHFormalParameter extends SimpleNode {
    ...
    protected BshParameter toParameter(CallStack callstack, Interpreter interpreter) throws EvalError {
        ...
        final Type baseType = !this.isLooseTyped() ? this.getTypeNode()._toType(callstack, interpreter) : null;
        // TODO: ver onde dá de usar esse Types.arrayType() ao invés de Array.newInstance(type, null).getClass()
        // TODO: ver isso do Types.getRawType()
        final Type type = this.isVarArgs ? bsh.Types.arrayType(Types.getRawType(baseType)) : baseType;
        final int modifiers = this.isFinal ? BshModifier.FINAL : 0;
        return new BshParameter(modifiers, type, this.name, baseType == null, this.isVarArgs);
    }
