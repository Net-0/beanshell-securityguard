...
class BSHArguments extends SimpleNode {
     ...
     * be compounded.
     */
    // TODO: faz sentido ter esse Node e método só para isso ?
    public Object[] getArguments(CallStack callstack, Interpreter interpreter) throws EvalError {
        ...
        for (int i = 0; i < args.length; i++) {
            args[i] = this.jjtGetChild(i).eval(callstack, interpreter);
            if (args[i] == Primitive.VOID)
                throw new EvalException("Undefined argument: " + this.jjtGetChild(i).getText(), this, callstack);
        }
